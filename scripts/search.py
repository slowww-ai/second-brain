...
Backlinks and note metadata live in wiki/index.db.
...
`--backlinks` reads only SQLite, so it works with no API key and no extra
packages installed. Semantic search and `--tag` require GEMINI_API_KEY (and
google-genai / lancedb) — they are imported lazily, only when actually used.
"""
